 ...
 *
 *
 *  + Person, Student, Worker, StudentWorker 클래스의 생성자, 복사 생성자, 소멸자
 *    내에서 출력 문장들 모두 주석 처리
 *  + 추상 클래스 Phone, Calculator, SmartPhone, BaseStation
 *  + 추상 클래스 BaseStation을 PersonManager에서 상속하여 구현
 *  + Person::smartPhone 멤버 및 관련 멤버 함수 추가
 *  + PersonManager::find(), dispPhones(), dispStudentWorkers() 추가
...
SmartPhone* Person::newSmartPhone(){
	SmartPhone *p;
	if(id%2==1)
		p = new GalaxyPhone(name);
	else
		p = new IPhone(name,"13");
	return p;
}
...
bool Student::operator==(const Person &p){
	...
	if(Person::operator==(p) && this->department == s.department && this->year == s.year)
			return 1;
	return 0;
}
void Student::study() {
...
StudentWorker::StudentWorker(
    ...
    const string& career, bool male):
			...
			Worker(name,id,weight,married,address,company,position),career{career},male{male}
...
StudentWorker::StudentWorker(const StudentWorker& a):
    /* TODO: 함수 서두에서 Person, Student, Worker의 [복사 생성자] 호출하기,
             a의 각 멤버를 this의 멤버에 복사하여 초기화 */Person(a),Student(a),Worker(a) { // 기존의 복사 생성자 참고할 것
    career = a.career;
    male = a.male;
	//cout << "StudentWorker::StudentWorker(const StudentWorker& a):"; printMembers(cout); cout << endl;
}
...
bool StudentWorker::operator==(const Person &p){
	...
	if(Student::operator==(p) && Worker::operator==(p) && this->male == a.male)
		return 1;
	return 0;
}
...
void CurrentUser::changePasswd() { //Menu item 8
...
void CurrentUser::run() {
    ...
    func_t func_arr[] = {
		...
		&CU::defaultParameter, &CU::staticMember,&CU::changeSmartPhone,&CU::calculate,
		&CU::phoneCall,&CU::calExpr,
    };
    int menuCount = sizeof(func_arr) / sizeof(func_arr[0]); // func_arr[] 배열의 길이
    string menuStr =
            "+++++++++++++++++++++ Current User Menu ++++++++++++++++++++++++\n"
...
public:
    ...
    void find();         		// Menu item 9
    void dispStudentWorkers(); 	// Menu item 10
    void dispPhones(); 			// Menu item 11
    bool connectTo(const string& caller, const string& callee) override;
    void run();
