...
public:
    // 기본 생성자 (타겟 생성자를 호출)
    Person() : Person("") {
        // name은 빈 문자열로 설정하고, 타겟 생성자로 위임
    }

    // 문자열만 받는 생성자 (타겟 생성자를 호출)
    Person(const string name) : Person(name, 0, 0.0, false, "") {
        // 초기화는 모든 멤버 변수를 기본값으로 설정하면서 진행
    }

    // 모든 멤버 변수를 초기화하는 타겟 생성자
    Person(const string name, int id, double weight, bool married, const char* address)
        ...
        setAddress(address); // 주소를 설정
        cout << "Person::Person(...):"; println(); // 출력 정보
    }
    ...
    void setAddress(const char* address) {
           ...
           this->address[sizeof(this->address) - 1] = '\0'; // 안전한 문자열 복사
       }
...
void Person::printMembers(ostream* pout)   {
	    cout << name <<" " <<  id << " " << weight << " " << (married? true : false) <<" :" << address << ":";
    // TODO: [문제 1]
}
...
VectorPerson::~VectorPerson() {
	for (int i = 0; i < count; ++i) {
	            delete pVector[i]; // 객체 삭제
	        }
	        delete[] pVector; // 배열 메모리 해제
	        pVector = nullptr; // 포인터를 null로 설정
    cout << "VectorPerson::~VectorPerson(): pVector deleted" << endl;
}

void VectorPerson::push_back(Person* p) {
    // 현재 배열이 가득 찬 경우, 배열 크기를 확장
    if (count == allocSize) {
        capacity();  // 배열 확장 로직 호출
    }
...
PersonManager::PersonManager(Person* array[], int len) {
    ...
    for (int i = 0; i < len; i++) {
    	Person* s = array[i];  // 현재 객체의 포인터 가져오기

              // 새로운 동적 Person 객체 생성 (복사 생성)
...
//Person gps("gps", 1000, 100, true, "gps address");
...
class ClassAndObject
{
    ...
    void global_static_local_objects_inner() {
        ...
        //static Person sps("sps", 2000, 200, false, "sps address");
    }
